...
const DOM = {
    ...
    filterContainer: document.querySelector('.shop-filters'),
...
async function loadShopConfig() {
    try {
        const response = await fetch(SHOP_CONFIG_ENDPOINT);
        if (response.ok) {
            const data = await response.json();
            shopConfig = data.shopCategories || { daily: {}, featured: {} };
        } else {
            shopConfig = { daily: { displayName: 'Daily' }, featured: { displayName: 'Featured' } };
        }
    } catch {
        shopConfig = { daily: { displayName: 'Daily' }, featured: { displayName: 'Featured' } };
    }
...
function generateDynamicUI() {
    if (!DOM.shopContent || !DOM.filterContainer) return;

    DOM.shopContent.innerHTML = '';

    const categories = Object.keys(shopConfig);
    categories.forEach(categoryKey => {
        const category = shopConfig[categoryKey];
        const displayName = category.displayName || categoryKey.charAt(0).toUpperCase() + categoryKey.slice(1);

        const section = document.createElement('div');
        section.className = 'shop-section';
        section.innerHTML = `
            <h2 class="section-title">${displayName} Items</h2>
            <div class="items-grid" id="${categoryKey}-items">
                <div class="loading-spinner-large"></div>
            </div>
        `;
        DOM.shopContent.appendChild(section);
    });

    DOM.filterContainer.innerHTML = '<button class="filter-btn active" data-filter="all">All Items</button>';
    categories.forEach(categoryKey => {
        const category = shopConfig[categoryKey];
        const displayName = category.displayName || categoryKey.charAt(0).toUpperCase() + categoryKey.slice(1);
        ...
        btn.dataset.filter = categoryKey;
        btn.textContent = `${displayName} Items`;
...
function displayShopItems(shopSections) {
    ...
    const categories = shopConfig ? Object.keys(shopConfig) : ['daily', 'featured'];
    ...
    Object.entries(shopSections).forEach(([key, item]) => {
        ...
        for (const categoryKey of categories) {
            if (key.startsWith(categoryKey)) {
                categorizedItems[categoryKey].push(itemData);
                break;
            }
        }
    });
    ...
    categories.forEach(categoryKey => {
        ...
        if (container) {
            displayItemsGrid(container, categorizedItems[categoryKey], categoryKey);
        }
    });
}

function displayItemsGrid(containerElement, items, categoryKey = 'daily') {
    ...
    containerElement.innerHTML = items.map(item => createItemCard(item, categoryKey)).join('');
}

function createItemCard(item, categoryKey = 'daily') {
        ...
        <div class="shop-item ${owned ? 'owned' : ''}" data-category="${category}" data-rarity="${itemRarity.toLowerCase()}">
            ...
            <div class="item-info">
                ...
                <div class="item-footer">
                    <div class="item-price"><i class="fas fa-coins"></i>${price.toLocaleString()}</div>
                    ${owned
                        ? `<button class="item-buy-btn item-buy-owned" disabled><i class="fas fa-check"></i> Owned</button>`
                        : `<button class="item-buy-btn" onclick="purchaseItem('${key}')"><i class="fas fa-shopping-cart"></i> Buy</button>`
                    }
                </div>
            </div>
        </div>
...
(function() {
    const saved = localStorage.getItem('neodyme_creator_code');
    if (saved) {
...
async function shopApplyCreatorCode() {
    const inp = document.getElementById('shop-creator-input');
    const fb  = document.getElementById('shop-creator-feedback');
    if (!inp || !fb) return;
    ...
    try {
        ...
        if (data.valid) {
            ...
            fb.className = 'creator-feedback valid';
        } else {
            localStorage.removeItem('neodyme_creator_code');
            fb.textContent = '✗ Code not found or inactive';
            fb.className = 'creator-feedback invalid';
        }
    } catch {
