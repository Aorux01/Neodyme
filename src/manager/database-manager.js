...
class DatabaseManager {
    ...
    static async updateAccountRole(accountId, role) {
        ...
        if (db.updateAccountRole) {
            return await db.updateAccountRole(accountId, role);
        }
        const roleMap = { 'player': 0, 'mod': 1, 'moderator': 1, 'dev': 2, 'developer': 2, 'admin': 3, 'owner': 4, 'server': 5 };
        const clientType = typeof role === 'number' ? role : (roleMap[role.toLowerCase()] ?? 0);
        return await db.setClientType(accountId, clientType);
    }

    static async setBanStatus(accountId, banned, reason = null) {
        const db = this.getDatabaseInstance();
        if (db.setBanStatus) {
            return await db.setBanStatus(accountId, banned, reason);
        }
        if (banned) {
            return await db.banAccount(accountId, reason ? [reason] : [], null);
        } else {
            return await db.unbanAccount(accountId);
        }
    }
    ...
    static getRoleName(clientType) {
        const db = this.getDatabaseInstance();
        if (db.getRoleName) {
            return db.getRoleName(clientType);
        }
        const roleNames = { 0: 'player', 1: 'moderator', 2: 'developer', 3: 'admin', 4: 'owner', 5: 'server' };
        return roleNames[clientType] || 'player';
    }

    static getRoleLevel(role) {
        const db = this.getDatabaseInstance();
        if (db.getRoleLevel) {
            return db.getRoleLevel(role);
        }
    ...
    static async getClientSettings(accountId, buildId) {
        return await this.getDatabaseInstance().getClientSettings(accountId, buildId);
    }

    static async getClientSettingsFile(accountId, buildId) {
        return await this.getDatabaseInstance().getClientSettingsFile(accountId, buildId);
    }

    static async saveClientSettings(accountId, buildId, content) {
        return await this.getDatabaseInstance().saveClientSettings(accountId, buildId, content);
    }

    static async deleteClientSettings(accountId, buildId) {
        return await this.getDatabaseInstance().deleteClientSettings(accountId, buildId);
    }
    ...
    static async addVbucks(accountId, amount) {
        const db = this.getDatabaseInstance();
        if (db.addVbucks) {
            return await db.addVbucks(accountId, amount);
        }
        throw new Error('Add V-Bucks not supported');
    }
    ...
    static async getAuditLogs() {
        return await this.getDatabaseInstance().getAuditLogs();
    }

    static async saveAuditLogs(auditData) {
        return await this.getDatabaseInstance().saveAuditLogs(auditData);
    }

    static async addAuditLog(logEntry) {
        return await this.getDatabaseInstance().addAuditLog(logEntry);
    }

    static async getTickets() {
        return await this.getDatabaseInstance().getTickets();
    }

    static async saveTickets(ticketsData) {
        return await this.getDatabaseInstance().saveTickets(ticketsData);
    }

    static async getTicket(ticketId) {
        return await this.getDatabaseInstance().getTicket(ticketId);
    }

    static async createTicket(ticket) {
        return await this.getDatabaseInstance().createTicket(ticket);
    }

    static async updateTicket(ticketId, updates) {
        return await this.getDatabaseInstance().updateTicket(ticketId, updates);
    }

    static async deleteTicketData(ticketId) {
        return await this.getDatabaseInstance().deleteTicket(ticketId);
    }

    static async getCreatorCodesData() {
        return await this.getDatabaseInstance().getCreatorCodesData();
    }

    static async saveCreatorCodesData(codesData) {
        return await this.getDatabaseInstance().saveCreatorCodesData(codesData);
    }

    static async getCreatorCode(code) {
        return await this.getDatabaseInstance().getCreatorCode(code);
    }

    static async createCreatorCode(codeData) {
        return await this.getDatabaseInstance().createCreatorCode(codeData);
    }

    static async updateCreatorCode(code, updates) {
        return await this.getDatabaseInstance().updateCreatorCode(code, updates);
    }

    static async deleteCreatorCode(code) {
        return await this.getDatabaseInstance().deleteCreatorCode(code);
    }

    static async getCreatorCodeRequest(requestId) {
        return await this.getDatabaseInstance().getCreatorCodeRequest(requestId);
    }

    static async createCreatorCodeRequest(request) {
        return await this.getDatabaseInstance().createCreatorCodeRequest(request);
    }
