...
class EndpointManager {
    ...
    static async start() {
        ...
        // Start periodic player/system stats recording
        try {
            const PlayerStatsService = require('../service/api/player-stats-service');
            ...
            const recordStats = async () => {
                try {
                    const accounts = await DatabaseManager.getAllAccounts().catch(() => []);
                    const totalAccounts = accounts.length;
                    const cutoff = Date.now() - 5 * 60 * 1000;
                    const activePlayers = accounts.filter(a =>
                        a.lastLogin && new Date(a.lastLogin).getTime() > cutoff
                    ).length;
                    await PlayerStatsService.record(activePlayers, totalAccounts, 0);
                } catch (err) {
                    LoggerService.log('error', `Stats recording error: ${err.message}`);
                }
            };

            recordStats();
            setInterval(recordStats, STATS_INTERVAL_MS);
        } catch (err) {
            LoggerService.log('warn', `Stats recording could not start: ${err.message}`);
        }
        ...
        try {
            ...
            if (ConfigManager.get('corsEnable', true)) {
                this.app.use(cors({
                    ...
                    allowedHeaders: ['Content-Type', 'Authorization', 'X-Epic-Correlation-ID', 'X-Requested-With', 'X-CSRF-Token', 'X-Session-ID']
                }));
            }
            ...
            const limit_request = ConfigManager.get('limitRequestSize', '1mb');
    ...
    static setupErrorHandling() {
        this.app.use((req, res, next) => {
            // Serve the HTML 404 page for browser navigation; keep JSON for API calls
            const isApiPath = req.path.startsWith('/api/')
                || req.path.startsWith('/fortnite/')
                || req.path.startsWith('/account/')
                || req.path.startsWith('/lightswitch/')
                || req.path.startsWith('/affiliate/')
                || (req.headers['accept'] || '').includes('application/json');

            if (!isApiPath) {
                const notFoundPage = path.join(__dirname, '..', '..', 'web', 'html', '404.html');
