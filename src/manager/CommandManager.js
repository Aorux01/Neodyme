...
class CommandManager {
    ...
    static registerDefaultCommands() {
        ...
        this.register('/op', async (args) => {
            if (args.length < 2) {
                LoggerService.log('warn', 'Usage: /op <enable|disable> <email|accountId>');
                return;
            }
            ...
            if (action !== 'enable' && action !== 'disable') {
                LoggerService.log('warn', 'Action must be "enable" or "disable"');
                return;
            }
        
            try {
                let account = DatabaseManager.getAccount(identifier);
                
                if (!account) {
                    account = DatabaseManager.getAccountByEmail(identifier);
                }
        
                if (!account) {
                    LoggerService.log('error', `Account not found: ${identifier}`);
                    return;
                }
        
                const isAdmin = action === 'enable';
                DatabaseManager.setAdmin(account.accountId, isAdmin);
                
                LoggerService.log('success', `Admin status ${action}d for: ${account.displayName} (${account.email})`);
            } catch (error) {
                LoggerService.log('error', `Failed to update admin status: ${error.message}`);
            }
        });
        ...
        this.register('/ban', async (args) => {
            if (args.length < 2) {
                LoggerService.log('info', `Usage: ${colors.cyan('/ban <username> <reason> [duration]')}`);
                LoggerService.log('info', 'Examples:');
                LoggerService.log('info', `  ${colors.cyan('/ban "Player123" "Cheating"')} - Permanent ban`);
                LoggerService.log('info', `  ${colors.cyan('/ban "Player123" "Toxicity" "7d"')} - 7 days ban`);
                LoggerService.log('info', `  ${colors.cyan('/ban "Player123" "Spamming" "2h"')} - 2 hours ban`);
                return;
            }
        ...
        this.register('/unban', async (args) => {
            if (args.length < 1) {
                LoggerService.log('info', `Usage: ${colors.cyan('/unban <username>')}`);
                return;
            }
        ...
        this.register('/baninfo', async (args) => {
            if (args.length < 1) {
                LoggerService.log('info', `Usage: ${colors.cyan('/baninfo <username>')}`);
                return;
            }
        ...
        this.register('/admin', async (args) => {
            if (args.length < 2) {
                LoggerService.log('info', `Usage: ${colors.cyan('/admin <username> <add|remove>')}`);
                LoggerService.log('info', 'Examples:');
                LoggerService.log('info', `  ${colors.cyan('/admin "Player123" add')} - Make Player123 an admin`);
                LoggerService.log('info', `  ${colors.cyan('/admin "Player123" remove')} - Remove admin rights from Player123`);
                return;
            }
            ...
            try {
                ...
                if (action === 'add') {
                    await DatabaseManager.setAdmin(account.accountId, true);
                    LoggerService.log('success', `Player "${username}" is now an administrator`);
                } else if (action === 'remove') {
                    await DatabaseManager.setAdmin(account.accountId, false);
                    LoggerService.log('success', `Player "${username}" is no longer an administrator`);
                } else {
                    LoggerService.log('error', 'Invalid action. Use "add" or "remove"');
                }
        
            } catch (error) {
                LoggerService.log('error', `Failed to modify admin rights: ${error.message}`);
            }
        });
        ...
        this.register('/player', async (args) => {
            ...
            try {
                ...
                LoggerService.log('info', `  Admin: ${account.admin ? colors.green('YES') : colors.red('NO')}`);
