...
class CommandManager {
    ...
    static registerDefaultCommands() {
        ...
        this.register('/admin', async (args) => {
            ...
            const ROLES = { PLAYER: 0, FOUNDER: 1, ADMIN: 2, MODERATOR: 3, HELPER: 4 };
            const ROLE_NAMES = { 0: null, 1: "[FOUNDER]", 2: "[ADMIN]", 3: "[MODERATOR]", 4: "[HELPER]" };
            const ROLE_COLORS = { 0: null, 1: colors.magenta, 2: colors.red, 3: colors.yellow, 4: colors.cyan };
            ...
            switch (subCommand) {
                case 'set':
                    if (args.length < 3) {
                        ...
                        LoggerService.log('info', `  1 - Founder ${colors.magenta('[FOUNDER]')}`);
                        LoggerService.log('info', `  2 - Admin ${colors.red('[ADMIN]')}`);
                        LoggerService.log('info', `  3 - Moderator ${colors.yellow('[MODERATOR]')}`);
                        LoggerService.log('info', `  4 - Helper ${colors.cyan('[HELPER]')}`);
                        return;
                    }
                    ...
                    try {
                        ...
                        const roleMap = { 'player': 0, 'founder': 1, 'admin': 2, 'moderator': 3, 'mod': 3, 'helper': 4 };
                        let roleId = !isNaN(roleInput) ? parseInt(roleInput) : roleMap[roleInput];
        
                        if (roleId === undefined || roleId < 0 || roleId > 4) {
                            LoggerService.log('error', 'Invalid role. Use 0-4 or role name');
                            return;
                        }
        
                        await DatabaseManager.setClientType(account.accountId, roleId);
                ...
                case 'list':
                    try {
                        ...
                        [ROLES.ADMIN, ROLES.MODERATOR, ROLES.HELPER, ROLES.FOUNDER].forEach(roleId => {
                ...
                default:
                    LoggerService.log('info', `Usage: ${colors.cyan('/admin <set|list>')}`);
                    LoggerService.log('info', 'Subcommands:');
                    LoggerService.log('info', `  ${colors.cyan('set')}  - Set a player's role`);
        ...
        // Ready check command (for container orchestration)
        ...
        // Account unlock command (for admins to manually unlock locked accounts)
