...
class ShopManager {
    ...
    async rotateShop() {
        try {
            ...
            const categories = this.config.shopCategories || {
                DailyItems: { count: this.config.shopDailyItemsCount || 6, displayName: 'Daily' },
                FeaturedItems: { count: this.config.shopFeaturedItemsCount || 8, displayName: 'Featured' }
            };

            const shopConfig = { "//": "BR Item Shop Config" };
            const categoryItems = {}; // Store items for each category

            // Process each category
            Object.keys(categories).forEach(categoryKey => {
                const category = categories[categoryKey];
                const count = category.count || 6;
                const items = this.pickRandomItems(cosmetics, count);

                // Determine the prefix (daily, featured, or custom)
                let prefix;
                if (categoryKey === 'DailyItems') {
                    prefix = 'daily';
                } else if (categoryKey === 'FeaturedItems') {
                    prefix = 'featured';
                } else {
                    // Format custom category name to lowercase, remove spaces
                    prefix = categoryKey.toLowerCase().replace(/\s+/g, '');
                }

                // Store items for this category
                categoryItems[prefix] = items;

                items.forEach((item, index) => {
                    shopConfig[`${prefix}${index + 1}`] = {
            ...
            // Add image URLs for all categories
            Object.keys(categoryItems).forEach(prefix => {
                const items = categoryItems[prefix];
                items.forEach((item, index) => {
                    shopState[`${prefix}${index + 1}`] = item.images?.smallIcon || item.images?.icon || null;
                });
            });
