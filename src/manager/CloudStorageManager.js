...
class CloudStorageManager {
    ...
    static generateDefaultEngine() {
        const gameServersPath = path.join(__dirname, '..', '..', 'config', 'GameServers.json');
        const xmppPort = ConfigManager.get('xmppPort') || 80;
...
`;
    ...
    static getSystemFile(fileName, season) {
        const filePath = path.join(this.SYSTEM_DIR, fileName);
        ...
        if (fileName === 'DefaultEngine.ini' && season >= 24) {
            let contentStr = content.toString('utf-8');
            if (!contentStr.includes('net.AllowEncryption=0')) {
