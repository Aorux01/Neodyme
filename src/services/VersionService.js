...
class VersionService {
    ...
    loadServerProperties() {
        const configPath = path.resolve(__dirname, '..', 'server.properties');
        try {
            if (fs.existsSync(configPath)) {
                ...
                this.customVersionEnabled = config.customVersion === 'true';
                this.customVersion = config.version || null;
                
                if (this.customVersion) {
                    this.customVersion = this.normalizeVersion(this.customVersion);
                    LoggerService.log('info', `Custom version enabled: ${this.customVersion}`);
                }
            }
        } catch (error) {
            LoggerService.log('error', `Error loading server.properties: ${error}`);
        }
    }
    ...
    getVersionInfo(req) {
        ...
        LoggerService.log('info', `Version info: Season ${versionInfo.season}, Build ${versionInfo.build}, CL ${versionInfo.CL}`);
        return versionInfo;
    }

    getVersionResponse() {
        const versionInfo = this.customVersionEnabled && this.customVersion ? 
            this.supportedVersions[this.customVersion] : 
            this.supportedVersions["18.30"];
        ...
        return {
            ...
            "cln": versionInfo.CL || "17951730",
            "build": versionInfo.build.toString() || "444",
            "moduleName": "Fortnite-Core",
            "buildDate": currentDate.toISOString(),
            "version": this.customVersion || "18.30",
            "branch": `Release-${this.customVersion || "18.30"}`,
            "modules": {
    ...
    getVersionCheckResponse() {
        if (this.customVersionEnabled) {
            const currentVersion = this.getVersionInfo();
            const targetVersion = this.customVersion;

            if (currentVersion.build.toString() !== targetVersion) {
                LoggerService.log('info', `Version mismatch: Current ${currentVersion.build}, Target ${targetVersion}`);
                return {
                    "type": "UPDATE_NEEDED",
                    "current": currentVersion.build.toString(),
    ...
    isVersionSupported(version) {
        return !!this.supportedVersions[version];
    }
    ...
    getSeasonVersions(season) {
        return Object.entries(this.supportedVersions)
            .filter(([version, info]) => info.season === season)
