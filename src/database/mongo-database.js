...
class MongoDatabase {
    ...
    static async getClientSettings(accountId, buildId) {
        const doc = await CloudStorage.findOne({ accountId });
        ...
        return {
            ...
            hash: crypto.createHash('sha1').update(doc.content).digest('hex'),
            hash256: crypto.createHash('sha256').update(doc.content).digest('hex'),
    ...
    static async saveClientSettings(accountId, buildId, content) {
        ...
        await CloudStorage.findOneAndUpdate(
            { accountId },
            { content: buffer, updatedAt: new Date() },
            { upsert: true }
        );
    ...
    static async deleteClientSettings(accountId, buildId) {
        const result = await CloudStorage.deleteOne({ accountId });
