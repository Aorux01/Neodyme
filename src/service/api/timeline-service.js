...
class TimelineService {
    getActiveEvents(versionInfo) {
        ...
        if (ConfigManager.get('bAllSTWEventsActivated')) {
    ...
    handleDynamicEvents(states, versionInfo, baseActiveEvents) {
        if (versionInfo.build === 4.5 && ConfigManager.get('bEnableGeodeEvent')) {
            const geodeStartDate = ConfigManager.get('GeodeEventStartDate');
            ...
            if (ConfigManager.get('bEnableCrackInTheSky')) {
                postEventState.push({
        ...
        if (versionInfo.build === 5.21) {
            if (ConfigManager.get('bEnableS5OddityPrecursor') === true) {
                ...
                states[1].activeEvents.push({
                    ...
                    "activeUntil": "9999-01-01T00:00:00.000Z",
                    "activeSince": ConfigManager.get('S5OddityPrecursorDate')
                });
            }
            ...
            if (ConfigManager.get('bEnableS5OddityExecution') === true) {
                ...
                states[states.length - 1].activeEvents.push({
                    ...
                    "activeUntil": "9999-01-01T00:00:00.000Z",
                    "activeSince": ConfigManager.get('S5OddityExecutionDate')
                });
            }
        }
    
        if (versionInfo.build === 5.30) {
            if (ConfigManager.get('') === true) {
            ...
            if (ConfigManager.get('bEnableCubeLightning') === true) {
                ...
                baseActiveEvents.push({
                    ...
                    "activeUntil": "9999-01-01T00:00:00.000Z",
                    "activeSince": ConfigManager.get('cubeSpawnDate'),
                });
        ...
        if (versionInfo.build === 5.41) {
            if (ConfigManager.get('bEnableCubeLake') === true) {
                ...
                states[1].activeEvents.push(
                    {
                        "eventType": "EventFlag.BR_S5_Cube_StartMove",
                        "activeUntil": ConfigManager.get('cubeLakeDate'),
                        "activeSince": ConfigManager.get('cubeLakeDate'),
                    },
                    {
                        "eventType": "EventFlag.BR_S5_Cube_TurnOn",
                        "activeUntil": ConfigManager.get('cubeLakeDate'),
                ...
                const eventEndDate = new Date(new Date(ConfigManager.get('cubeLakeDate')).getTime() + 1.5 * 60000).toISOString();
