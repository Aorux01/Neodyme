...
class PagesService {
    ...
    static generateShopSections() {
        ...
        const sections = [];
        let priorityCounter = 70;

        for (const [key, categoryConfig] of Object.entries(shopConfig.shopCategories)) {
            const sectionId = categoryConfig.displayName || key.charAt(0).toUpperCase() + key.slice(1);
            ...
            sections.push({
                ...
                "_type": "ShopSection",
                "landingPriority": priorityCounter,
                "bHidden": false,
                "sectionId": sectionId,
                "bShowTimer": true,
                "sectionDisplayName": categoryConfig.displayName || sectionId,
    ...
    static getSparkTracks() {
        const sparkTracksPath = path.join(process.cwd(), 'content', 'pages', 'spark-tracks.json');
    ...
    static getRadioStations() {
        const radioPath = path.join(process.cwd(), 'content', 'pages', 'radio-stations.json');
    ...
    static getContentPages(req) {
        ...
        const baseContentPagesPath = path.join(process.cwd(), 'content', 'pages', 'content-pages.json');
        ...
        try {
            if (fs.existsSync(configDir)) {
                ...
                files.forEach(file => {
                    if (file.endsWith('.json') && file !== 'content-pages.json') {
        ...
        try {
            ...
            if (dynamicShopSections.length > 0) {
                ...
                contentPages.shopSections.sectionList.sections = dynamicShopSections;
            }
        } catch (error) {
            LoggerService.log('error', `Failed to generate dynamic shop sections: ${error.message}`);
        }
