...
class TokenService {
    ...
    static async initialize() {
        ...
        LoggerService.log('info', `TokenService initialized (storage: ${RedisManager.isEnabled() ? 'Redis' : 'JSON'})`);
    ...
    static async storeToken(type, hash, data, expiresInSeconds) {
        if (RedisManager.isEnabled()) {
            ...
            await RedisManager.client.setex(key, expiresInSeconds, JSON.stringify(data));
        } else {
    ...
    static async updateTokenActivity(token) {
        ...
        if (tokenData) {
            ...
            if (RedisManager.isEnabled()) {
                ...
                if (ttl > 0) {
                    await RedisManager.client.setex(key, ttl, JSON.stringify(tokenData));
                }
            } else {
