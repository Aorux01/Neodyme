...
router.get("/fortnite/api/cloudstorage/user/:accountId", verifyToken, async (req, res) => {
    try {
        ...
        const clientSettings = await DatabaseManager.getClientSettings(accountId, version.build);
...
router.get("/fortnite/api/cloudstorage/user/*/:file", verifyToken, async (req, res) => {
    try {
        const fileName = req.params.file;
        ...
        const version = VersionService.getVersionInfo(req);
        const content = await DatabaseManager.getClientSettingsFile(accountId, version.build);
...
router.put("/fortnite/api/cloudstorage/user/*/:file", verifyToken, async (req, res) => {
    try {
        const fileName = req.params.file;
        ...
        await DatabaseManager.saveClientSettings(accountId, version.build, req.rawBody);
